...
import eclipsectlmcp.mcp.McpServerDescriptor.EnvironmentVariable;
...
public class McpServerPreferencePage extends PreferencePage implements IWorkbenchPreferencePage
{
    ...
    {
        int selectedIndex = serverTable.getSelectionIndex();
        if (selectedIndex < 0 || getSelectedServerIsBuiltIn())
        {
            super.performApply();
            return;
        }
        ...
        McpServerDescriptor updatedServer = new McpServerDescriptor( "", nameText.getText(), commandText.getText(), null, currentEnvVars, true,  false ); 

        presenter.saveServer( selectedIndex, updatedServer );
        super.performApply();
    }

    private boolean getSelectedServerIsBuiltIn()
    {
        var selected = presenter.getServerAt(serverTable.getSelectionIndex());
        return selected.map(McpServerDescriptor::builtIn).orElse(false);
    }
