...
public class ConsoleService
{
    ...
    /**
     * Safely logs an error message, handling cases where the logger bundle is null (e.g., in tests).
     */
    private void safeLogError(String message, Throwable e) {
        try {
            if (logger != null && logger.getBundle() != null) {
    ...
    /**
     * Safely logs an info message, handling cases where the logger bundle is null (e.g., in tests).
     */
    ...
    public void println(String consoleName, String message)
    {
        ...
        sync.syncExec(() -> {
            try {
                ...
                safeLogError("Error writing to console: " + e.getMessage(), e);
            }
        });
    }
        ...
        sync.syncExec(() -> {
            try {
                ...
                try {
                    ...
                    safeLogError("Failed to show console view", e);
                }
                
                safeLogInfo("Console '" + consoleName + "' cleared successfully.");
            } catch (Exception e) {
                safeLogError("Error clearing console: " + e.getMessage(), e);
            }
        });
    }
    /**
